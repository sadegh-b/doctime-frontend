...
export default function AppointmentsPage() {
  ...
  const loadAppointments = () => {
    try {
      ...
      console.error("خطا در خواندن نوبت‌ها:", error);
      setAppointments([]);
    }
  };

  const cancelAppointment = (id) => {
    const confirmed = window.confirm("آیا می‌خواهی این نوبت را لغو کنی؟");
    if (!confirmed) return;
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-2">
              این صفحه نوبت‌هایی را نشان می‌دهد که از صفحه پزشک ثبت کرده‌ای.
            </p>
          </div>
        ...
        {appointments.length === 0 ? (
          <div className="text-center py-16 bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
            <p className="text-slate-500 font-bold mb-4">هنوز نوبتی ثبت نشده است.</p>
            <Link
              to="/doctors"
